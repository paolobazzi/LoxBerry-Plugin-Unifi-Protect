...
# Copyright 2026 Paolo Bazzi, loxberry@bazzi.biz
#
...
my $log = LoxBerry::Log->new ( 
	name => 'Unifi Protect',
	filename => "$lbplogdir/unifi-protect.log",
	append => 1
	);
LOGSTART "Unifi protect start";
...
$CONTROLLER_URL = $pcfg->param("CONTROLLER.URL");

print "Configuration:<br>";
print "- Controller URL: ".$CONTROLLER_URL."<br>";

our $countcameras = 0;
#our $rowsradios;

my %cameraconfig = $pcfg->vars();	

$CAMERA_0 = ''; 

foreach my $key (keys %cameraconfig) {
	if ( $key =~ /^CAMERA/ ) {
		my @fields = $pcfg->param($key);
		
#		print "Camera (key): ".$key."<br>";
		print "- Camera[".$countcameras."]: ".$fields[0]."<br>";
		
		$CAMERA_0 = $fields[0];
		
#		$countcameras++;
#		LOGDEB "Camera: ".$fields[0];
#		print "Camera: ".$fields[0];
	}
}

print "Found ".$countcameras." cameras<br>";
...
my $cgi = CGI->new;
$cgi->import_names('R');
$ENABLE_RECORDING= $R::action eq "enable-recording";
$DISABLE_RECORDING= $R::action eq "disable-recording";

LOGDEB "Action: ".$R::action;

if ($ENABLE_RECORDING || $DISABLE_RECORDING) {
	print "Enable recording<br>";
	
	my $login_url = $CONTROLLER_URL.'/api/auth/login';
	my $login_header = ['Content-Type' => 'application/json'];
	my $req = HTTP::Request->new('POST', $login_url, $login_header, '{"username" : "protect","password" : "changeme"}' );
...
#		if ($TEST_MODE) {
			...
			$x_csrf_token = $response->header('x-updated-csrf-token');
			
			printf "Header".$x_csrf_token."<br>";
#		}
...
#			printf "</p>";
		...
		my $url = $CONTROLLER_URL.'/proxy/protect/api/cameras/'.$CAMERA_0;
		my $header = ['Content-Type' => 'application/json', 'X-Csrf-Token' => $x_csrf_token];
		
		my $req;
		
		if ($ENABLE_RECORDING) {
			$req = HTTP::Request->new('PATCH', $url, $header, '{"recordingSettings": {"mode": "always"}}');
			print "Enable recording<br>";
		} else {
			$req = HTTP::Request->new('PATCH', $url, $header, '{"recordingSettings": {"mode": "never"}}');
			print "Disable recording<br>";
		}
...
#			LOGDEB "Trigger action id=".$ID.", value=".$VALUE." successful, message: ".$response->message;
		print "Trigger action successful, message: ".$response->message."<br>";
		} else {
#			LOGDEB "Trigger action id=".$ID.", value=".$VALUE." failed. Error Code: ".$response->code.", Message: ".$response->message;
		print "Trigger action failed. Error Code: ".$response->code.", Message: ".$response->message."<br>";
		}
		exit 0;	
...
#		}
	...
	print "No or invalid action specified<br>";
}
...
LOGDEB "Operation finished sucessfully.";
